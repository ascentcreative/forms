...
var Code = {
  ...
  _init: function _init() {
    var self = this;
    this.element.addClass('initialised'); //allow textareas (with code class) to accept tabs:
    ...
    $(document).delegate('.code-editor', 'keydown', function (e) {
      ...
      if (keyCode == 9) {
        ...
        var end = $(this).get(0).selectionEnd; // set textarea value to: text before caret + tab + text after caret
...
var ContentEditable = {
  ...
  _init: function _init() {
    ...
    var charlimit = $(this.element).data('charlimit');
    ...
    $(this.element).on('paste', '.fce-edit', function (e) {
      ...
      pastedData = clipboardData.getData('Text'); // paste the text only content in at the cursor position:
...
var DateTimeField = {
  _init: function _init() {
    ...
    this.element.addClass('initialised'); // alert('DTF');
    // 

    console.log(this.element);
    $(this.element).on('change', function (e) {
      console.log('dtf change');
      var out = $(self.element).find('.dtf-date').val() + " " + $(self.element).find('.dtf-time').val();
      console.log(out);
      $(self.element).find('.dtf-output').val(out); // if($out != '--') {
      //     $(self.element).find('.compound-date-output').val($out);
      // } else {
      //     $(self.element).find('.compound-date-output').val(null);
      // }
...
var FormRedirect = {
  ...
  _init: function _init() {
    ...
    var thisID = this.element[0].id;
    var fldName = idAry[1];
    var obj = this.element; // alert('initing FormRedirect');
...
$.extend($.ascent.FormRedirect, {}); // init on document ready
...
var RelatedTokens = {
  ...
  _init: function _init() {
    ...
    console.log(self.options); // Create an autocompleete widget
  ...
  addToken: function addToken(label) {
    ...
    if (label == '') return; // check if token exits. Don't allow duplicates.
...
var observer = new MutationObserver(function (mutations, observer) {
  ...
  // console.log(mutations, observer);
  // ...
  $('.subform-loader').not('.initialised').subformloader(); // if (.removedNodes) console.log(1);
