...
@section('element')
    ...
    @else
        ...
        <div style="display: flex; align-items: center; height: 100%;" class="pt-1">
            <input type="radio" id="{{$name}}-{{$checkedValue}}" name="{{$name}}" value="{{$checkedValue}}"
            ...
            @if($labelAfter)
                <label class="flex-label-text pl-3 mb-0" for="{{$name}}-{{$checkedValue}}">@if($labelEscape){{$label}}@else{!! $label !!}@endif</label>
            @endif
        </div>
